...
# write for 100 days, 200 days
# write for 20kg, 100kg, 200kg


def diffCrossdep(N,Z,E,T,M,sin2thetaw, phi):
    g_vp = (1/2-2*(sin2thetaw) -2*phi)
    g_vn = -1/2
    return ((G_f)**2/(np.pi))*((g_vn*N-g_vp*Z)**2)*M*(1-(M*T)/(2*(E**2)))
...
def fluxdep(T, sin2thetaw,phi):        
      return integrate.quad(lambda E: diffCrossdep(N,Z,E,T,Ar_m,sin2thetaw,phi)*SpecTot(E), np.sqrt(Ar_m * T /2), E_max)[0]
...
for mass in masses.keys():
    ...
    total_targets = times["100 días"]*masses[mass]*(3.8E-10)

    Ntheo = total_targets * integrate.quad(lambda t: fluxdep(t, weibergAngle,phi_nuel), 30*(1/1000)*(1/1000),Tmax)[0]
    
    print(Ntheo)

    angles = np.linspace(phi_nuel-0.0007,phi_nuel+0.0007,500)
    Nexp = []

    for i in angles:
        Nexp.append(total_targets*integrate.quad(lambda t: fluxdep(t, weibergAngle, i), 30*(1/1000)*(1/1000),Tmax)[0])
...
for key in chiSqR.keys():
    ...
    if lim1[key] > lim2[key]:   
        ax.plot(angles, chiSqR[key], label = key + r" $\phi_{\nu_e W} = "+f"{minAngle[key]:.5f}"+r"^{+"+f"{lim1[key] - minAngle[key]:.5f}"
                           +r"}_{"+f"-{minAngle[key] - lim2[key]:.5f}"+r"}$")
    else:
        ax.plot(angles, chiSqR[key], label = key + r" $\phi_{\nu_e W} = "+f"{minAngle[key]:.5f}"+r"^{+"+f"{lim2[key] - minAngle[key]:.5f}"
...
plot.suptitle(r"Diferencia estadística $\chi^2$ para diferenetes correcciones radiativas $\phi_{\nu_e W}$ en CE$\nu$NS de neutrinos "+ "\n"+ r" provenientes de reactores nucleares interactuando con diferentes massas de $^{40}$Ar por 100 días")

ax.legend()
plt.savefig("/home/nullpost/Scripts/college stuf/CEvNS sequel/Figure_phi_100.png",dpi=200)
# print(Tmax)
