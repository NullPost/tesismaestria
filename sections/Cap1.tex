...
\end{equation}
Later refinements of this theory succeeded in demonstrating the universality of the weak interaction across leptonic decays and strangeness-changing meson decays through Cabibbo mixing \cite{Cabibbo:1963yz}. This not only established that all such processes are mediated by the same underlying force, but also motivated the prediction and subsequent discovery of the charm quark \cite{Glashow:1970gm}. With these improvements on the theory thus far, the point-like structure of the interaction still led to scattering amplitudes that where proportional to center-of-mass energy $\sqrt{s}$ and to probability violation at higher energies.  In direct analogy with QED, where the point-like Coulomb interaction is resolved into the exchange of a massless photon, the Fermi vertex can be interpreted as the low-energy limit of the exchange of a charged spin-1 $W^{\pm}$, wherein a mass is considered for such a mediator in order to explain the short range of weak interactions.
