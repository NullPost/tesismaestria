...
The expression (\ref{eq:factoroftwo}) can be shown to be a correction that adds coherently to the tree-level weak neutral current amplitude, since both couple left-handed neutrinos to the same conserved vector current of the target. The tree-level weak neutral current vertex involves the product $G_F g_V$, where $g_V$ is the vector coupling of the target fermion to the $Z$, and the propagator structure of the weak amplitude at low energies effectively replaces $q^2/M_Z^2 \to 0$ at leading order. The electromagnetic amplitude, by contrast, carries an explicit factor of $q^2$ from $f_Q(q^2)$ which cancels the photon propagator $1/q^2$, leaving a finite, $q^2$-independent residue at leading order. The interference of this residue with the weak amplitude then shifts the effective vector coupling as
%
\begin{equation}\label{eq:gVshift}
g_V \to g_V + \frac{2\sqrt{2}\pi\alpha}{3G_F}\langle r^2\rangle,
\end{equation}
%
where $\alpha$ is the fine-structure constant~\cite{Cadeddu_2018,Vogel_Engel_1989}. Since $g_V^p = T^p_f - 2Q^p\sin^2\theta_W= \frac{1}{2} - 2\sin^2\theta_W$ for the proton, this shift can equivalently be absorbed into a renormalization of the weak mixing angle,
%
\begin{equation}\label{eq:sin2shift}
\sin^2\theta_W \to \sin^2\theta_W\left(1 + \frac{1}{3}M_W^2\langle r^2\rangle\right),
\end{equation}
%
which is the form most commonly quoted in the literature~\cite{Cadeddu_2018,Vogel_Engel_1989,Barranco_2008}. The two shifts (\ref{eq:gVshift}) and (\ref{eq:sin2shift}) are equivalent at leading order in $\alpha$ and their derivation makes transparent why the NCR promotes $\sin^2\theta_W$ into a running coupling: the charge form factor $f_Q(q^2)$ carries the full $q^2$ dependence of the electromagnetic loop, and once this is included in the effective amplitude the vector coupling acquires a momentum-transfer dependence that is absent at tree level. In this sense the measurement of the NCR via CE$\nu$NS is precisely a measurement of the running of the weak mixing angle from the $Z$ pole down to the momentum transfers accessible at reactor and spallation neutron source experiments, wherein the SM prediction (\ref{eq:NCRSMpred}) fixes the expected magnitude and sign of the effect.
